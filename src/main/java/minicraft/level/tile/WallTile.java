...
public class WallTile extends Tile {
	...
	protected WallTile(Material type) {
		...
		switch (type) {
			...
			case Basalt: sprite = new ConnectorSprite(WallTile.class, new Sprite(10, 18, 3, 3, 1, 3), new Sprite(13, 18, 2, 2, 1, 3), new Sprite(11, 19, 2, 2, 1, 0, true)); break;
		}
		csprite = sprite;
	}
	...
	public void hurt(Level level, int x, int y, int dmg) {
		...
		if (damage >= sbwHealth) {
			...
			switch (type) { // Get what tile to set and what item to drop
				...
				case Basalt: {
					itemName = "Basalt Brick";
					tilename = "Basalt Bricks";
					break;
				}
			}

			level.dropItem(x * 16 + 8, y * 16 + 8, 1, 3 - type.ordinal() > 0 ? 3 - type.ordinal() : 1, Items.get(itemName));
