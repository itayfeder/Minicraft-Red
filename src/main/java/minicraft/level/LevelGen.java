...
public class LevelGen {
	...
	static short[][] createAndValidateMap(int w, int h, int level) {
		...
		if (level == 2)
			return createAndValidateNetherMap(w, h);
	...
	public static @Nullable short[][] createAndValidateNetherMap(int w, int h) {
		random.setSeed(worldSeed);

		do {
			short[][] result = createNetherMap(w, h);
			...
			if (count[Tiles.get("Netherrock").id & 0xffff] < 100) continue;
			if (count[Tiles.get("Soul Sand").id & 0xffff] < 100) continue;
			if (count[Tiles.get("Basalt").id & 0xffff] < 100) continue;
	...
	public static short[][] createNetherMap(int w, int h) {
		// creates a bunch of value maps, some with small size...
		LevelGen mnoise1 = new LevelGen(w, h, 16);
		LevelGen mnoise2 = new LevelGen(w, h, 16);
		LevelGen mnoise3 = new LevelGen(w, h, 16);
		...
		for (int y = 0; y < h; y++) {
			for (int x = 0; x < w; x++) {
				...
				if (val < -0.5) {
					...
					map[i] = Tiles.get("Basalt").id;
				} else {
					map[i] = Tiles.get("Netherrock").id;
				}
			}
		}
			...
			for (int k = 0; k < 10; k++) {
				...
				for (int j = 0; j < 100; j++) {
					...
					for (int yy = yo - 1; yy <= yo + 1; yy++)
						for (int xx = xo - 1; xx <= xo + 1; xx++)
							if (xx >= 0 && yy >= 0 && xx < w && yy < h) {
								if (map[xx + yy * w] == Tiles.get("netherrock").id) {
									map[xx + yy * w] = Tiles.get("soul sand").id;
								}
							}
				}
			}
...
//			int yy = random.nextInt(h);
//			if (xx >= 0 && yy >= 0 && xx < w && yy < h) {
//				if (map[xx + yy * w] == Tiles.get("soul sand").id) {
//					map[xx + yy * w] = Tiles.get("bone block").id;
//				}
//			}
//		}
