...
public class Level {
	...
	private static final String[] levelNames = {"Nether", "Sky", "Surface", "Iron", "Gold", "Lava", "Dungeon"};
	public static String getLevelName(int depth) { return levelNames[-1 * depth + 2]; }
	...
	public Level(int w, int h, long seed, int level, Level parentLevel, boolean makeWorld) {
		...
		} else if (depth == 1) { // This is a sky level
	...
	private void trySpawn() {
		...
		for (int i = 0; i < 30 && !spawned; i++) {
			...
			if ((Updater.getTime() == Updater.Time.Night && Updater.pastDay1 || depth != 0) && depth != 2 && EnemyMob.checkStartPos(this, nx, ny)) { // if night or underground, with a valid tile, spawn an enemy mob.
			...
			if (depth == 2 && EnemyMob.checkStartPos(this, nx, ny)) { // if nether
				...
				if (rnd <= 40) add((new Piglin(1)), nx, ny);
	...
	private void generateNetherSpawners() {
		for (int i = 0; i < 18 / 2 * (w / 128); i++) {
			...
			int x3 = random.nextInt(16 * w) / 16;
			int y3 = random.nextInt(16 * h) / 16;
			if (getTile(x3, y3) == Tiles.get("netherrock")) {
				boolean xaxis2 = random.nextBoolean();

				if (xaxis2) {
					for (int s2 = x3; s2 < w - s2; s2++) {
						if (getTile(s2, y3) == Tiles.get("basalt")) {
							sp.x = s2 * 16 - 24;
							sp.y = y3 * 16 - 24;
						}
					}
					...
					for (int s2 = y3; s2 < y3 - s2; s2++) {
						if (getTile(x3, s2) == Tiles.get("basalt")) {
							sp.x = x3 * 16 - 24;
							sp.y = s2 * 16 - 24;
						}
					}
				...
				if (getTile(sp.x / 16, sp.y / 16) == Tiles.get("basalt")) {
					setTile(sp.x / 16, sp.y / 16, Tiles.get("netherrock"));
				}

				Structure.netherDungeonCenter.draw(this, sp.x / 16, sp.y / 16);

				if (getTile(sp.x / 16, sp.y / 16 - 4) == Tiles.get("netherrock")) {
					Structure.netherDungeonNorth.draw(this, sp.x / 16, sp.y / 16 - 5);
				}
				if (getTile(sp.x / 16, sp.y / 16 + 4) == Tiles.get("netherrock")) {
					Structure.netherDungeonSouth.draw(this, sp.x / 16, sp.y / 16 + 5);
				}
				if (getTile(sp.x / 16 + 4, sp.y / 16) == Tiles.get("netherrock")) {
					Structure.netherDungeonEast.draw(this, sp.x / 16 + 5, sp.y / 16);
				}
				if (getTile(sp.x / 16 - 4, sp.y / 16) == Tiles.get("netherrock")) {
					Structure.netherDungeonWest.draw(this, sp.x / 16 - 5, sp.y / 16);
				}
				...
				for (int rpt = 0; rpt < 2; rpt++) {
					...
					c.populateInvRandom("netherdungeon", chance);
