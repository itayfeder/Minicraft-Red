...
public class Furniture extends Entity {
	...
	public boolean interact(Player player, @Nullable Item item, Direction attackDir) {
		...
		else if (item instanceof ToolItem && this instanceof ObsidianPortal) {
			ToolItem toolItem = (ToolItem) item;
			if (toolItem.type == ToolType.Flint_And_Steel) {
				if (3 == Game.currentLevel)
					((ObsidianPortal) this).lit = true;
				else {
					Game.notifications.add("Portal can only be opened on ground level!");
				}
			}
