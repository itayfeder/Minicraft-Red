...
public abstract class Mob extends Entity {
	...
	public void tick() {
		...
		if (level != null && level.getTile(x >> 4, y >> 4) == Tiles.get("lava") && !fireResistant) {
			ignite(101);
			hurt(Tiles.get("lava"), x, y, 4);
		}
		if (level != null && level.getTile(x >> 4, y >> 4) == Tiles.get("water")) {
			ignite(0);
		}
		...
		if (igniteTime > 0) {
			if (igniteTime % 20 == 0) {
				...
				for (int i = 0; i < 3; i++) {
					...
					int randY = random.nextInt(16) - 8;
					level.add(new FireParticle(x + randX, y - 6 + randY));
				}
			}
			igniteTime--;
			if (health <= 0) burnedOut = true;
		}
	...
	protected boolean updateField(String field, String val) {
		...
		switch (field) {
			...
			case "igniteTime": igniteTime = Integer.parseInt(val); return true;
	...
	public int getLightRadius() {
		if (igniteTime > 0) return super.getLightRadius() + 2;
