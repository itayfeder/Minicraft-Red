	...
	*/
	...
	public void renderRow(int r, Screen screen, int x, int y, boolean fullbright) {
		...
		for (int c = 0; c < row.length; c++) { // Loop across through each column
			screen.render(x + c * 8, y, row[c].sheetPos, row[c].mirror, row[c].sheet, -1, fullbright); // Render the sprite pixel.
		}
	}
	...
	public void renderRow(int r, Screen screen, int x, int y, int color) {
		Px[] row = spritePixels[r];
		for (int c = 0; c < row.length; c++) { // Loop across through each column
			screen.render(x + c * 8, y, row[c].sheetPos, row[c].mirror, row[c].sheet, -1, false, color); // Render the sprite pixel.
		}
	}
}
